...
### The resolver is not the only place the effective group is adopted

Filtering inside `GetUserUsableGroups` is correct for the query "which groups may this requester pick", but it does not, on its own, settle the awkward case where the requester's **own** user group is admin-only. The reason is that the resolver is consulted for the **token's** group at the enforcement points; the requester's own group is adopted as the effective group at several points that never ask the resolver whether the requester may use it. Relying on the filter to have already removed the group from the map is what the first cut of this change did, and it leaked in four places: `middleware/auth.go` skips the usable-group check entirely when `token.Group` is the default empty string and then puts the owner's group on the context; `middleware/distributor.go` lets a playground request name the requester's own group and short-circuits the check; `service/channel_select.go` selects channels by the token group with no role check; and `controller/model.go` `getModelListGroups` returns the effective group's models without reading the effective group at all.

The rule therefore lives in exactly one exported predicate, `service.IsAdminOnlyGroupForRole(group, role)`, read as "this group is withheld from a requester at this role by the admin-only configuration". Two kinds of caller use it: the resolver's final filter, and the enforcement points that adopt an effective group. The gate itself sits as far upstream as the shape of the code allows — in `middleware/auth.go` `TokenAuth`, immediately before `ContextKeyUsingGroup` is written, using the key owner's role and the existing forbidden-group response. Everything downstream that reads the effective group inherits correctness from it: the token's named group was already checked, and an ungrouped token's inherited group is now checked too. The playground path reaches `middleware/distributor.go` through `UserAuth`, not `TokenAuth`, and its effective group is the requester's own user group placed on the context by `setDashboardAuthContext`, so the same predicate is applied there at the point where the group starts selecting channels. `controller/model.go` `getModelListGroups` never reads `ContextKeyUsingGroup` — it resolves the group from `ContextKeyTokenGroup` and `ContextKeyUserGroup` on its own — so it applies the predicate to the resolved non-auto group, yielding no models rather than inventing a new error response.

This is deliberately gated **only** on the admin-only predicate. A broader "the effective group must be in `GetUserUsableGroups`" check would newly refuse users whose own group was removed by a `GroupSpecialUsableGroup` `-:` override, which is a behaviour change this change does not own and `proposal.md` excludes.

The consequence for a non-administrator assigned to an admin-only group is a deliberate choice of consistency over convenience — the alternative, exempting the requester's own group, means the flag means "hidden unless you are assigned to it", which is not a security property and is much harder to explain. The consequence is stated in the settings UI where the flag is set, so an operator meets it before it bites; the UI does not query how many users sit in the group, because that is a database round trip on a settings screen to soften a warning that is true regardless of the count.
