...
export function HeroTerminalDemo(props: HeroTerminalDemoProps) {
  ...
  return (
    <div className={cn('mx-auto w-full max-w-2xl', props.className)}>
      <div
        className={cn(
          'border-border bg-card overflow-hidden rounded-2xl border shadow-[0_26px_60px_-25px_rgba(58,42,140,0.14)] backdrop-blur-sm'
        )}
      >
        {/* Tab strip */}
        <div
          className={cn(
            'border-border flex items-center gap-1 border-b px-2 sm:gap-1.5 sm:px-3'
          )}
        >
        ...
        <div
          className={cn(
            'border-border flex items-center gap-2.5 border-b px-5 py-3'
          )}
        >
          <span
            className={cn(
        ...
        <div
          className={cn(
            'border-border bg-muted/40 flex items-center justify-between border-t px-5 py-2.5'
          )}
        >
...
function ResponseBlock(props: { demo: ApiDemoConfig; transitioning: boolean }) {
  ...
  return (
    <div
      className={cn('border-border bg-muted/20 relative border-t px-5 py-4')}
    >
...
function Command(props: { children: ReactNode }) {
  return (
    <span className='text-accent-foreground font-medium'>{props.children}</span>
  )
}

function Flag(props: { children: ReactNode }) {
  return <span className='text-chart-3'>{props.children}</span>
}
